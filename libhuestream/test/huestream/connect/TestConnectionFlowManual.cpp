...
public:
    ...
    void connect_manual_ip_starts_pushlink(int index) {
    ...
    void set_manual_retrieves_full_config(int index, int numGroups, int numBridges = 1, bool wasPreviousBridgeConnected = false, bool wasPreviousBridgeStreaming = false, int previousBridgeIndex = 0) {
        expect_message(FeedbackMessage::ID_USERPROCEDURE_STARTED, FeedbackMessage::FEEDBACK_TYPE_INFO);
    ...
    void reset_bridge_returns_empty_bridge(bool wasPreviousBridgeConnected = false, bool resetAllBridges = false, int numberOfBridges = 1) {
        expect_message(FeedbackMessage::ID_USERPROCEDURE_STARTED, FeedbackMessage::FEEDBACK_TYPE_INFO);
        if (wasPreviousBridgeConnected) {
            expect_message(FeedbackMessage::ID_BRIDGE_DISCONNECTED, FeedbackMessage::FEEDBACK_TYPE_INFO);
        }
...
TEST_P(TestConnectionFlow_Manual, connect_with_manual_ip) {
    ...
    connect_manual_ip_starts_pushlink(0);
...
TEST_P(TestConnectionFlow_Manual, connect_with_manual_ip_and_credentials) {
    _settings->SetAutoStartAtConnection(int2bool(GetParam()));
    set_manual_retrieves_full_config(1, 1);
...
TEST_P(TestConnectionFlow_Manual, reset_bridge_which_was_connected_vs_not_connected) {
    ...
    reset_bridge_returns_empty_bridge(wasBridgeConnected);
...
TEST_P(TestConnectionFlow_Manual, reset_single_bridge_vs_reset_all_bridges) {
    ...
    set_manual_retrieves_full_config(2, 1, 2, true, false, 1);
    finish(_bridges->at(2));

    auto resetAllBridges = int2bool(GetParam());
    if (resetAllBridges) {
        reset_bridge_returns_empty_bridge(true, resetAllBridges);
    } else {
        reset_bridge_returns_empty_bridge(true, resetAllBridges, 2);
    }
...
TEST_F(TestConnectionFlow_Manual, connect_with_manual_ip_followed_by_connect_with_manual_ip_and_credentials) {
    ...
    set_manual_retrieves_full_config(1, 2, 2, true, false, 0);
    finish_without_stream_start(false);
}
...
TEST_F(TestConnectionFlow_Manual, connect_with_manual_ip_abort_and_retry) {
    ...
    connect_manual_ip_starts_pushlink(2);
...
TEST_P(TestConnectionFlow_Manual, add_manual_already_known_bridge_doesnt_duplicate_entry) {
    ...
    set_manual_retrieves_full_config(0, 1);
    finish(_bridges->at(0));

    set_manual_retrieves_full_config(1, 1, 2, true, false, 0);
    finish(_bridges->at(1));

    set_manual_retrieves_full_config(0, 1, 2, true, false, 1);
    finish(_bridges->at(0));

    set_manual_retrieves_full_config(2, 1, 3, true, false, 0);
    finish(_bridges->at(2));

    set_manual_retrieves_full_config(1, 1, 3, true, false, 2);
    finish(_bridges->at(1));

    set_manual_retrieves_full_config(0, 1, 3, true, false, 1);
    finish(_bridges->at(0));
}
...
TEST_P(TestConnectionFlow_Manual, add_manual_when_already_streaming_first_stops_streaming) {
    ...
    set_manual_retrieves_full_config(1, 1, 2, true, true, 0);
    finish(_bridges->at(1));
}
