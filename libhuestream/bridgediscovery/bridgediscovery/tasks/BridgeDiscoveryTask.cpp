...
#include "bridgediscovery/tasks/BridgeDiscoveryTask.h"

#include <unordered_set>
#include <utility>
#include "../IBridgeDiscoveryCallback.h"
#include "BridgeDiscoveryCheckIpArrayTask.h"
...
namespace huesdk {
    BridgeDiscoveryTask::BridgeDiscoveryTask(vector<unique_ptr<IBridgeDiscoveryMethod>> &&discovery_methods)
            : _executor(huesdk_lib::global_execution_queue()) {
    ...
    vector<BridgeDiscoveryResult> BridgeDiscoveryTask::get_results() const {
    ...
    void BridgeDiscoveryTask::start_next_search_method() {
        if (!_current_discovery_methods.empty()) {
            ...
            discovery_method->search(make_shared<BridgeDiscoveryCallback>([this, perform_ip_check](const vector<BridgeDiscoveryResult> &discovery_results, BridgeDiscoveryReturnCode /*return_code*/) {
    ...
    void BridgeDiscoveryTask::perform_ip_check_and_continue(const vector<BridgeDiscoveryResult> &discovery_results) {
        ...
        for (const auto &result_entry : discovery_results) {
            ...
            if (none_of(_results.begin(), _results.end(), [result_id](const pair<string, BridgeDiscoveryResult> &existing_pair) {
        ...
        check_ip_job->run([this](BridgeDiscoveryCheckIpArrayTask* task) {
            vector<BridgeDiscoveryResult> discovery_ipcheck_results;
            for (const auto &result_entry : task->get_result()) {
                discovery_ipcheck_results.emplace_back(result_entry.unique_id, result_entry.ip, result_entry.api_version, result_entry.model_id);
            }
    ...
    void BridgeDiscoveryTask::process_results_and_continue(const vector<BridgeDiscoveryResult> &discovery_results) {
        if (!_current_discovery_methods.empty()) {
            for (const auto &result_entry : discovery_results) {
