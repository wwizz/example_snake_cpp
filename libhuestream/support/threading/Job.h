...
#ifndef THREADING_JOB_H_
#define THREADING_JOB_H_
...
namespace huesdk_lib {
    ...
    public:
         ...
         * @return state of the job after method has completed
         */
        JobState run(std::function<void(TTask*)> callback);
    ...
    private:
        ...
        JobState run(std::shared_ptr<JobTask> task, std::function<void(JobTask*)> callback);
    ...
    template<typename TTask>
    JobState Job<TTask>::cancel() {
        {
            std::lock_guard<decltype(_state->mutex)> state_lock(_state->mutex);
            if (_state->job_state != JobState::Running && _state->job_state != JobState::Canceling) {
                return _state->job_state;
            }
        ...
        {
            std::lock_guard<decltype(_state->mutex)> state_lock(_state->mutex);
            if (_state->job_state != JobState::Completed) {
                _state->job_state = JobState::Canceled;

                _executor->execute([]{}, false);  // notify about required break from recursive operational loop inside Job::Wait
            }
    ...
    JobState Job<TTask>::wait() {
        {
            ...
            bool recursion_denied = !queue->run_recursive_process_tickets_loop([this] () -> bool {
                std::lock_guard<decltype(_state->mutex)> state_lock(_state->mutex);
                return (_state->job_state != JobState::Running && _state->job_state != JobState::Canceling);
            });

            if (recursion_denied) {
                std::unique_lock<decltype(_state->mutex)> state_lock(_state->mutex);
                _state->cv.wait(state_lock, [this] () -> bool {
                    return (_state->job_state != JobState::Running && _state->job_state != JobState::Canceling);
                });
            }
        }
    ...
    template<typename TTask>
    JobState Job<TTask>::run(std::shared_ptr<JobTask> task, std::function<void(JobTask*)> callback) {
        std::lock_guard<decltype(_state->mutex)> state_lock(_state->mutex);
        ...
        if (it_existing == std::end(_state->task_details)) {
            ...
            return _state->job_state;  // already running
        }
    ...
    template <typename TTask>
    JobState Job<TTask>::run() {
        std::function<void(TTask*)> callback;
        return run(callback);
    }

    template <typename TTask>
    JobState Job<TTask>::run(std::function<void(TTask*)> callback) {
    ...
    std::function<void()> Job<TTask>::wrap_task_execution(std::shared_ptr<JobTask> task, std::function<void(JobTask*)> callback) {
        ...
        const auto notify_done = [callback, weak_task, state] {
        ...
        const auto repost_callback = [weak_executor, notify_done, state] {
            std::lock_guard<decltype(state->mutex)> state_lock(state->mutex);
            if (state->job_state == JobState::Running) {
